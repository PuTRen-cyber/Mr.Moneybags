...
- 当前只允许 Phase 2C：独立项目会话、有限意图提取、歧义检测、决策归属和对齐门槛，不提前实现 Phase 2D/2E/3。
...
- 本次 Phase 2C 请求仅授权验证通过后的一个本地项目提交，提交信息为 `feat: add project conversation and intent alignment`，完成后停止；不构成后续提交授权。边界测试只在独立临时目录创建 Git fixtures。
