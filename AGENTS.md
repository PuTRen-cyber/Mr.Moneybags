...
- 当前只允许 Phase 5：Rolling Planning、Current Work Unit、Stage Briefing 与 Agent Task Package，不提前实现 Phase 6。v5.0 编号映射保留，Phase 5 遵循 v5.1 产品方向，不重做已接受实现或改写历史。
...
- 历史规格和计划通过显式版本关联替代，保留旧含义；不新增 Prompt、Git 提交自动化或持久化。
...
- 本次 Phase 5 请求仅授权验证通过后的一个本地项目提交，提交信息为 `feat: add rolling planning and agent task packages`，完成后停止；不构成后续提交授权。边界测试只在独立临时目录创建 Git fixtures。
